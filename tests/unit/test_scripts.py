...
def test_the_release_script_covers_every_application_image() -> None:
    ...
    # and is not one of ours. `embedder`, `modelfetch` and `scheduler` are the
    # worker image under a different command — one copy of 2.3 GB of weights is
    # the whole design (`P2-17`, `B-14`), and the scheduler spawns `python -m`
    # entry points out of that same tree (`B-15`) — so none of them is a
    # separate thing to push.
    built_by_compose -= {"crawl4ai", "embedder", "modelfetch", "scheduler"}
